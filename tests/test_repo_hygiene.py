...
DEFERRED_MODULES = ("sentence_transformers",)
...
def test_the_package_imports_without_the_evidence_extras(monkeypatch):
    ...
    or `pip install .` followed by `chunkaudit sweep` breaks on a machine that
...
def test_the_deferred_dependency_is_imported_somewhere_inside_a_function():
    """The other half: it has to actually be reachable, not merely absent."""
    tree = ast.parse((REPO / "src" / "chunkaudit" / "retrieval.py").read_text(encoding="utf-8"))
    found = False
