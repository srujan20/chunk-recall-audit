...
WARMUP = [sys.executable, "-m", "chunkaudit", "--version"]
...
SEGMENTS = (
    ("The plan, and the one retriever this build could never run", ["plan"]),
    (
        "A 200 character chunking, a common default. Note the ceiling",
        ["audit", "--size", "200", "--stride", "200"],
    ),
    (
        "Eight hundred characters instead. Better, and still not good",
        ["audit", "--size", "800", "--stride", "800"],
    ),
    (
        "Whole document chunks: a perfect ceiling, and a cost the ceiling cannot show",
        ["audit", "--strategy", "document"],
    ),
    (
        "Every chunking in the plan, sorted worst ceiling first",
        ["sweep", "--retrievers", "bm25", "--ks", "5"],
    ),
)
...
def capture(label: str, argv: list[str], *, module: bool) -> Captured:
    command = f"python -m chunkaudit {' '.join(argv)}" if module else f"python {' '.join(argv)}"
    full = [sys.executable, "-m", "chunkaudit", *argv] if module else [sys.executable, *argv]
