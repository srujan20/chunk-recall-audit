...
Tooling and CI only. **129 figures held** and **0 figures moved**, which is
the claim this release is really making: what the repository finds did not change,
only how thoroughly it is checked.

### Added

- The composite action now runs in CI, twice, asserting the verdict and the exit
  code for each one its inputs can reach. Its third verdict,
  `causes-cannot-be-told-apart`, fires when a containing chunk is missing from a
  ranking, which is a retriever bug rather than a property of a chunking, so no
  input reaches it and the workflow names the test that covers it instead.
...
- Every published figure. `tools/compare_releases.py --since v1.0.0` reports
  129 held and 0 moved, and the comparison reads both sides out of git.
- Every experiment, corpus and threshold. No policy value moved.

## [1.0.0]

First release. 129 machine checked figures, 233 tests, 99.2 percent
